...
namespace QFrameWork
{
    ...
    public abstract class ListWidget : Widget, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement
    {
        [SerializeField]
        ...
        public RectTransform content { get { return Content as RectTransform; } set { Content = value; }}
...
//        public ScrollRectEvent onValueChanged { get { return m_OnValueChanged; } set { m_OnValueChanged = value; } }
        ...
        {
            if (Application.isPlaying && this.isActiveAndEnabled && m_itemPool != null)
            {
                itemTypeEnd = itemTypeStart;
                // recycle items if we can
                for (int i = 0; i < content.childCount; i++)
                {
                    RectTransform temp = (RectTransform)content.GetChild(i);
                    if (itemTypeEnd < totalCount)
                    {
                        ProvideData(temp, i + itemTypeStart);
                        itemTypeEnd++;
                    }
                    else
                    {
                        ReturnObjectAndSendMessage(temp);
                        i--;
                    }
                }
            }
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (!Application.isPlaying || totalCount < 0 || contentConstraintCount > 1 || m_item == null || m_itemPool == null)
                return;
        ...
        {
            if (!Application.isPlaying || m_item == null || m_itemPool == null)
                return;
